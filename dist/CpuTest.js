...
const memory = new Memory_1.Memory(true); // Assuming 'true' enables debug mode // Remove the "_1" suffix
const mmu = new MMU_1.MMU(memory);
const cpu = new Cpu_1.Cpu(true);
cpu.setMMU(mmu);
/* AD LDA MEMORY
const program = [
...
mmu.memoryDump(0x0000, 0x000A); // Dump memory from 0x0000 to 0x000A
*/
function powersProgram() {
    mmu.writeImmediate(0x0000, 0xA9);
    mmu.writeImmediate(0x0001, 0x00);
    ...
    mmu.writeImmediate(0x0016, 0x50);
    mmu.writeImmediate(0x0017, 0x00);
    // test DO (Branch Not Equal) will be NE and branch 
    mmu.writeImmediate(0x0018, 0xD0);
    mmu.writeImmediate(0x0019, 0xED);
    // globals
    mmu.writeImmediate(0x0050, 0x2C);
    mmu.writeImmediate(0x0052, 0x00);
    // Your powersProgram code here...
}
// Run the powersProgram
powersProgram();
...
//# sourceMappingURL=CpuTest.js.map